...
if isinstance(path, list):
    files = sum([glob(os.path.expanduser(f"~/datasets/{args.data[:args.data.find(".")]}/{p}/*")) for p in path], [])
else:
    files = glob(os.path.expanduser(f"~/datasets/{args.data[:args.data.find(".")]}/{path}/*"))
